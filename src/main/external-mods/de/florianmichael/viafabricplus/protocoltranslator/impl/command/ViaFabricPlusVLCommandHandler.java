...
public class ViaFabricPlusVLCommandHandler extends VLCommandHandler {
    ...
    public int execute(CommandContext<?> ctx) {
        ...
        onCommand(
                new ViaFabricPlusViaCommandSender((SharedSuggestionProvider) ctx.getSource()),
                args
        );
        return 1;
    }

    public CompletableFuture<Suggestions> suggestion(CommandContext<?> ctx, SuggestionsBuilder builder) {
        String[] args;
        try {
        ...
        onTabComplete(new ViaFabricPlusViaCommandSender((SharedSuggestionProvider) ctx.getSource()), args)
                .stream()
                .map(it -> {

      ...
      switch (this) {
         ...
         case RANDOM:
            if (args.length > 0) {
               ...
               return (float)Math.abs(j1) / (float)Integer.MAX_VALUE;
            }
         ...
         case TIME:
            ...
            return (float)(level.getGameTime() % 720720L) + GameRenderer.getRenderPartialTicks();
         case DAY_TIME:
            if (level == null) {
               return 0.0F;
            }
   ...
   public boolean evalBool(IExpression[] args) {
      switch (this) {
         ...
         case PRINTB:
            ...
            if (FrameEvent.isActive("CEM-PRINTB-" + i, j)) {
               Config.dbg("CEM printb(" + i + ") = " + flag);
            }

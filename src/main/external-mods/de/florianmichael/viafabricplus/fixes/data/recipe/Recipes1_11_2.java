...
public class Recipes1_11_2 {
    ...
    public static List<RecipeInfo> getRecipes(final ProtocolVersion targetVersion) {
        ...
        if (targetVersion.newerThanOrEqualTo(LegacyProtocolVersion.r1_4_2)) {
            recipes.add(RecipeInfo.of(() -> new ArmorDyeRecipe(CraftingBookCategory.MISC)));
            recipes.add(RecipeInfo.of(() -> new MapCloningRecipe(CraftingBookCategory.MISC)));
            recipes.add(RecipeInfo.of(() -> new MapExtendingRecipe(CraftingBookCategory.MISC)));
        }
        if (targetVersion.newerThanOrEqualTo(LegacyProtocolVersion.r1_4_6tor1_4_7)) {
            recipes.add(RecipeInfo.of(() -> new FireworkRocketRecipe(CraftingBookCategory.MISC)));
        }
        if (targetVersion.newerThanOrEqualTo(ProtocolVersion.v1_11)) {
            recipes.add(RecipeInfo.of(() -> new ShulkerBoxColoring(CraftingBookCategory.MISC)));
        }
        if (targetVersion.newerThanOrEqualTo(ProtocolVersion.v1_9)) {
            recipes.add(RecipeInfo.of(() -> new TippedArrowRecipe(CraftingBookCategory.MISC)));
            recipes.add(RecipeInfo.of(() -> new ShieldDecorationRecipe(CraftingBookCategory.MISC)));
        }
        if (targetVersion.newerThanOrEqualTo(ProtocolVersion.v1_8)) {
            recipes.add(RecipeInfo.of(() -> new RepairItemRecipe(CraftingBookCategory.MISC)));
            recipes.add(RecipeInfo.of(() -> new BannerDuplicateRecipe(CraftingBookCategory.MISC)));
            recipes.add(RecipeInfo.of(() -> new AddBannerPatternRecipe(CraftingBookCategory.MISC)));
        }
        if (targetVersion.newerThanOrEqualTo(ProtocolVersion.v1_7_2)) {
            recipes.add(RecipeInfo.of(() -> new BookCloningRecipe(CraftingBookCategory.MISC)));
        }
    ...
    public static void setCraftingResultSlot(final int syncId, final AbstractContainerMenu screenHandler, final CraftingContainer inventory) {
        final var network = Minecraft.getInstance().getConnection();
        final var world = Minecraft.getInstance().level;

        final var result = network.getRecipeManager()
                .getRecipeFor(RecipeType.CRAFTING, inventory, world) // Get the first matching recipe
                .map(recipe -> recipe.value().assemble(inventory, network.registryAccess())) // Craft the recipe to get the result

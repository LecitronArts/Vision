      ...
      return (locIn, resIn) -> {
         ...
         if (Mth.isMultipleOf(nativeimage.getWidth(), framesize.width()) && Mth.isMultipleOf(nativeimage.getHeight(), framesize.height())) {
            if (Reflector.ForgeHooksClient_loadSpriteContents.exists()) {
               SpriteContents spritecontents = (SpriteContents)Reflector.ForgeHooksClient_loadSpriteContents.call(locIn, resIn, framesize, nativeimage, resourcemetadata);
            ...
            LOGGER.error("Image {} size {},{} is not multiple of frame size {},{}", locIn, nativeimage.getWidth(), nativeimage.getHeight(), framesize.width(), framesize.height());
            nativeimage.close();
            return null;
         }
      };

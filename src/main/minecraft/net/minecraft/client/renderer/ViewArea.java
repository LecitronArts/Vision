...
public class ViewArea {
         ...
         for(int k = 0; k < this.sectionGridSizeX; ++k) {
            for(int l = 0; l < this.sectionGridSizeY; ++l) {
               for(int i1 = 0; i1 < this.sectionGridSizeZ; ++i1) {
                  ...
                  this.sections[j1] = pSectionRenderDispatcher.new RenderSection(j1, k * 16, this.level.getMinBuildHeight() + l * 16, i1 * 16);
   ...
   public SectionRenderDispatcher.RenderSection getRenderSectionAt(BlockPos pPos) {
      int i = pPos.getY() - this.level.getMinBuildHeight() >> 4;
      if (i >= 0 && i < this.sectionGridSizeY) {
         int j = Mth.positiveModulo(pPos.getX() >> 4, this.sectionGridSizeX);
         int k = Mth.positiveModulo(pPos.getZ() >> 4, this.sectionGridSizeZ);
   ...
   public void clearUnusedVbos() {
      ...
      for(int j1 = Math.min(i1 + j, this.sections.length); i1 < j1 && l < k; ++i1) {
         ...
         ClearVertexBuffersTask clearvertexbufferstask = ClearVertexBuffersTask.make(sectionrenderdispatcher$rendersection.getCompiled().getLayersUsed(), sectionrenderdispatcher$rendersection);

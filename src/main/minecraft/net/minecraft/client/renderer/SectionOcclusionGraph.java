...
public class SectionOcclusionGraph {
   ...
   private void scheduleFullUpdate(boolean pSmartCull, Camera pCamera, Vec3 pCameraPosition) {
      ...
      this.fullUpdateTask = Util.backgroundExecutor().submit(() -> {
         ...
         queue.forEach((nodeIn) -> {
            sectionocclusiongraph$graphstate.storage.sectionToNodeMap.put(nodeIn.section, nodeIn);
         });
   ...
   private void initializeQueueForFullUpdate(Camera pCamera, Queue<SectionOcclusionGraph.Node> pNodeQueue) {
      ...
      if (sectionrenderdispatcher$rendersection == null) {
         ...
         list.sort(Comparator.comparingDouble((nodeIn) -> {
            return blockpos.distSqr(nodeIn.section.getOrigin().offset(8, 8, 8));
         }));
         pNodeQueue.addAll(list);
      } else {
         pNodeQueue.add(sectionrenderdispatcher$rendersection.getRenderInfo((Direction)null, 0));
      }
         ...
         for(Direction direction : DIRECTIONS) {
            ...
            if (sectionrenderdispatcher$rendersection1 != null && (!pSmartCull || !sectionocclusiongraph$node.hasDirection(direction.getOpposite()))) {
               ...
               if (pSmartCull && flag) {
                  ...
                  Vec3M vec3m = pGraphStorage.vec3M1.set((double)blockpos2.getX() + (double)b1, (double)blockpos2.getY() + (double)b2, (double)blockpos2.getZ() + (double)b0);
                  Vec3M vec3m1 = pGraphStorage.vec3M2.set(pCameraPosition).subtract(vec3m).normalize().scale(CEILED_SECTION_DIAGONAL);
                  boolean flag2 = true;

                  while(pGraphStorage.vec3M3.set(pCameraPosition).subtract(vec3m).lengthSquared() > 3600.0D) {
                     ...
                     SectionRenderDispatcher.RenderSection sectionrenderdispatcher$rendersection2 = this.viewArea.getRenderSectionAt(pGraphStorage.blockPosM1.setXyz(vec3m.x, vec3m.y, vec3m.z));
               ...
               if (sectionocclusiongraph$node1 != null) {
                  ...
                  SectionOcclusionGraph.Node sectionocclusiongraph$node2 = sectionrenderdispatcher$rendersection1.getRenderInfo(direction, sectionocclusiongraph$node.step + 1);
                  ...
                  if (sectionrenderdispatcher$rendersection1.hasAllNeighbors()) {
                     ...
                     pGraphStorage.sectionToNodeMap.put(sectionrenderdispatcher$rendersection1, sectionocclusiongraph$node2);
                     pGraphStorage.chunksWaitingForNeighbors.computeIfAbsent(ChunkPos.asLong(sectionrenderdispatcher$rendersection1.getOrigin()), (posLongIn) -> {
   ...
   private SectionRenderDispatcher.RenderSection getRelativeFrom(BlockPos pPos, SectionRenderDispatcher.RenderSection pSection, Direction pDirection) {
      ...
      if (blockpos.getY() >= clientlevel.getMinBuildHeight() && blockpos.getY() < clientlevel.getMaxBuildHeight()) {
         if (Mth.abs(pPos.getY() - blockpos.getY()) > this.levelRenderer.renderDistance) {
            return null;
         } else {

...
public class EntitySectionStorage<T extends EntityAccess> {
   ...
   public LongSet getAllChunksWithExistingSections() {
      ...
      this.sections.keySet().forEach((p_156884_1_) -> {
   ...
   public void getEntities(AABB pBounds, AbortableIterationConsumer<T> pConsumer) {
      this.forEachAccessibleNonEmptySection(pBounds, (sectionIn) -> {
         return sectionIn.getEntities(pBounds, pConsumer);
      });
   }

   public <U extends T> void getEntities(EntityTypeTest<T, U> pTest, AABB pBounds, AbortableIterationConsumer<U> pConsumer) {
      this.forEachAccessibleNonEmptySection(pBounds, (sectionIn) -> {
         return sectionIn.getEntities(pTest, pBounds, pConsumer);
      });
   }

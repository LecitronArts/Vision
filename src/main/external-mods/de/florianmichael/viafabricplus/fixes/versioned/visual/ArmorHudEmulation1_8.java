...
public class ArmorHudEmulation1_8 {
    ...
    private static void sendArmorUpdate(final UserConnection userConnection) throws Exception {
        ...
        int armor = 0;
        for (final ItemStack stack : Minecraft.getInstance().player.getInventory().armor) {
            armor += ArmorType.findByType(BuiltInRegistries.ITEM.getKey(stack.getItem()).toString()).getArmorPoints();
        }
        ...
        entityProperties.write(Type.VAR_INT, Minecraft.getInstance().player.getId());

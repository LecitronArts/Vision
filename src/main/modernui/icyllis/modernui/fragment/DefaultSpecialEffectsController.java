...
class DefaultSpecialEffectsController extends SpecialEffectsController {
    ...
    void executeOperations(@Nonnull List<Operation> operations, boolean isPop) {
        ...
        for (final Operation operation : operations) {
            State currentState = State.from(operation.getFragment().mView);
            switch (operation.getFinalState()) {
                case GONE:
                case INVISIBLE:
                case REMOVED:
                    if (currentState == State.VISIBLE && firstOut == null) {
    ...
    private void startAnimations(@Nonnull List<AnimationInfo> animationInfos,
        ...
        for (final AnimationInfo animationInfo : animationInfos) {
            ...
            final boolean isHideOperation = operation.getFinalState() == State.GONE;
            if (isHideOperation) {
    ...
    private static class SpecialEffectsInfo {
        ...
        boolean isVisibilityUnchanged() {
            State currentState = State.from(
                    mOperation.getFragment().mView);
            State finalState = mOperation.getFinalState();
            return currentState == finalState || (currentState != State.VISIBLE
                    && finalState != State.VISIBLE);
        }
    ...
    private static class AnimationInfo extends SpecialEffectsInfo {
        ...
        Animator getAnimator() {
            ...
            mAnimator = loadAnimator(getOperation().getFragment(),
                    getOperation().getFinalState() == State.VISIBLE, mIsPop);
            mLoadedAnim = true;
            return mAnimator;
        }
